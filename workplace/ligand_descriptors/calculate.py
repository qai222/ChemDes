...
if __name__ == '__main__':
    ...
    if not os.path.isfile(mols_file):
        Molecule.write_smi(mols, "mols.smi")  # write smi file
    mordred_df = calculate_mordred(smis=[m.smiles for m in mols])
    cxcalc_df = calculate_cxcalc(mol_file=mols_file)
    pka_df = opera_pka("mols-smi_OPERA2.7Pred.csv")
    ...
    des_df.to_csv("molecular_descriptors_{}.csv".format(datetime.datetime.now().strftime("%Y_%m_%d")), index=False)
